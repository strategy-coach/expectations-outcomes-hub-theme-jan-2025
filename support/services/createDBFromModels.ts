...
exec(`sqlite3 ${databasePath} ""`, (error, _stdout, stderr) => {
    ...
    exec(`sqlite3 ${databasePath} ".read ${sqlFilePath}"`, (error, _stdout, stderr) => {
        ...
        exec(`mv src/content/db/models/models.db src/content/db/`, (error, _stdout, stderr) => {
            if (error) {
            ...
            exec(`rm -rf src/content/db/models/models.auto.sql`, (error, _stdout, stderr) => {
                ...
                exec(`cd src/content/db && surveilr admin merge -p "activity%" -p "message%" -p "communication%" -p "contact%" -p "channel%" -p "reaction%" -p "attachment%" && rm -rf resource-surveillance.sqlite.db && mv resource-surveillance-aggregated.sqlite.db resource-surveillance.sqlite.db`, (error, _stdout, stderr) => {
                    if (error) {
